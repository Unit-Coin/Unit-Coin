...
UTVER=($($UNITCLI --version | head -n1 | awk -F'[ -]' '{ print $6, $7 }'))
...
for cmd in $UNITD $UNITCLI $UNITTX $UNITQT; do
  cmdname="${cmd##*/}"
  help2man -N --version-string=${UTVER[0]} --include=footer.h2m -o ${MANDIR}/${cmdname}.1 ${cmd}
  sed -i "s/\\\-${UTVER[1]}//g" ${MANDIR}/${cmdname}.1
done
